          ...
          {ADDONS.map((a, idx) => {
            ...
            return (
              <Reveal key={a.title} delay={idx * 80}>
                <div className="h-full rounded-2xl bg-white border border-slate-200 p-6 text-center">
                  <div className="w-12 h-12 rounded-xl bg-[#eef3fc] text-blue-500 flex items-center justify-center mx-auto mb-5">
                    <Icon className="w-6 h-6" />
                  </div>
                  <h3 className="font-display text-base font-bold text-slate-900 mb-2">
                    {a.title}
                  </h3>
                  <p className="text-sm text-slate-600 font-light leading-relaxed">{a.desc}</p>
                </div>
              </Reveal>
            );
          })}
