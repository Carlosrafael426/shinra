      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-14">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {COMPANY_METRICS.map((metric, idx) => (
            <div
              key={idx}
              className="p-6 rounded-2xl bg-slate-900 border border-slate-800 relative group hover:border-blue-500/50 transition-all duration-200"
            >
              <p className="text-3xl sm:text-4xl font-extrabold font-display text-white tracking-tight mb-1 group-hover:text-blue-400 transition-colors">
                {metric.value}
              </p>
      ...
      <div className="relative w-full overflow-hidden py-4">
        ...
        <div className="animate-marquee flex gap-4">
          {duplicatedTech.map((tech, index) => (
            <div
              key={index}
              className="flex items-center gap-3 px-5 py-3 rounded-xl bg-slate-900 border border-slate-800 hover:border-blue-500/50 hover:bg-slate-850 transition-all text-sm font-medium text-slate-200 whitespace-nowrap shadow-sm group"
            >
